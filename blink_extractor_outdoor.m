...
for s = 1:numel(names)
    if s == 1
        ...
        laure.CorrelationThreshold = 0.86;
    ...
    elseif s == 2
        ...
        kevin.CorrelationThreshold = 0.92;
    ...
    r = outdoorSubjects(s).gettrainingrecordingindex;
    
    %retrieve smoothed Model and its variance
    m = outdoorSubjects(s).Modelblink;
    [m.AverageOn, m.AverageOff, m.VarianceOn, m.VarianceOff] = outdoorSubjects(s).Recordings{r}.getmodelblink(30);
    ...
    if 1 == 1
        [blinksOn, blinksOff] = outdoorSubjects(s).Recordings{r}.getallblinks();
