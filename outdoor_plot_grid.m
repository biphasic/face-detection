...
for s = 1:length(outdoorSubjects)
    ...
    for r = 1:numel(outdoorSubjects(s).Recordings)
        ax = subplot(length(outdoorSubjects),length(outdoorSubjects(1).Recordings),(s-1)*3 + r);
    %for r = 1:numel(outdoorSubjects(s).Recordings)
    %    if ~isempty(outdoorSubjects(s).Recordings{r}) && outdoorSubjects(s).Recordings{r}.IsTrainingRecording
    %        break
    %    end
    %end
        grid1 = outdoorSubjects(s).Recordings{r}.EventstreamGrid1;
        corrThreshold = outdoorSubjects(s).CorrelationThreshold;
        ...
        hold on
        title(ax, outdoorSubjects(s).Name);

        grid2 = outdoorSubjects(s).Recordings{r}.EventstreamGrid2;
